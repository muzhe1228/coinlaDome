module common {
    export class HttpUtils {
        private ajax: Laya.HttpRequest;
        private apiBaseUrl: string = "http://192.168.10.200:7001";
        private callback: any;
        private caller: any;
        private decrypt: boolean;
        constructor(decrypt: boolean) {
            this.ajax = new Laya.HttpRequest();
            this.ajax.http.timeout = 10000;
            this.decrypt = decrypt;
            //this.http.once(Laya.Event.PROGRESS, this, this.onRequestProgress);
            
            this.ajax.on(Laya.Event.COMPLETE, this, this.onRequestComplete);
            this.ajax.on(Laya.Event.ERROR, this, this.onRequestError);
        }

        /**
         * 
         * @param actual 
         */
        private cleanArray(actual): any[] {
            const newArray = []
            for (let i = 0; i < actual.length; i++) {
                if (actual[i]) {
                    newArray.push(actual[i])
                }
            }
            return newArray;
        }

        /**
         * JSON对象转换为QueryString
         * @param jsonObj 
         */
        private toQueryString(jsonObj): string {
            if (!jsonObj) return '';
            var jsonString = this.cleanArray(
                Object.keys(jsonObj).map(key => {
                    if (jsonObj[key] === undefined) return ''
                    return encodeURIComponent(key) + '=' +
                        encodeURIComponent(jsonObj[key])
                })
            ).join('&')

            return jsonString;
        }

        /**
         * 发送get请求接口
         * @param apiPath 
         * @param params 
         */
        public sendGet(apiPath: string, params: any, caller: any, callback: any, contentType?: string): void {
            this.caller = caller;
            this.callback = callback;
            contentType = contentType == null ? CommonConsts.ContentType_JSON : contentType;

            var url: string = this.apiBaseUrl + apiPath;
            var data: string = this.toQueryString(params);

            this.ajax.send(url, data, CommonConsts.HttpMethod_GET, "json", ["Content-Type", CommonConsts.ContentType_JSON]);
        }

        /**
         * 发数post请求接口
         * @param apiPath 
         * @param params 
         */
        public sendPost(apiPath: string, params: any, caller: any, callback: any, contentType?: string): void {
            this.caller = caller;
            this.callback = callback;
            contentType = contentType == null ? CommonConsts.ContentType_JSON : contentType;

            var url: string = this.apiBaseUrl + apiPath;
            var data: string = contentType == CommonConsts.ContentType_FormData ? this.toQueryString(params) : JSON.stringify(params);

            this.ajax.send(url, data, CommonConsts.HttpMethod_POST, "json", ["Content-Type", contentType]);
        }
        /**
         * 发数post请求接口加密
         * @param apiPath 
         * @param params 
         */
        public sendPostKey(apiPath: string, params: any, caller: any, callback: any): void {
            this.caller = caller;
            this.callback = callback;

            var url: string = this.apiBaseUrl + apiPath;
            params = this.encryption(params)
            var data: string = JSON.stringify(params);

            this.ajax.send(url, data, CommonConsts.HttpMethod_POST, "json", ["Content-Type", CommonConsts.ContentType_JSON]);
        }
        //加解密
        private cryCommon(): void {
            CryptoJS.pad.ZeroPadding = {
                pad: function (data, blockSize) {
                    // Shortcut
                    var blockSizeBytes: any = blockSize * 4;

                    // Pad
                    data.clamp();
                    data.sigBytes +=
                        blockSizeBytes - (data.sigBytes % blockSizeBytes || blockSizeBytes);
                },

                unpad: function (data) {
                    // Shortcut
                    var dataWords: any = data.words;

                    // Unpad
                    var i = data.sigBytes - 1;
                    while (!((dataWords[i >>> 2] >>> (24 - (i % 4) * 8)) & 0xff)) {
                        i--;
                    }
                    data.sigBytes = i + 1;
                }
            };
        }

        private randomString(len: number): string {
            len = len || 32;
            var $chars: string =
                "ABCDEFGHJKMNPQRSTWXYZabcdefhijkmnprstwxyz2345678"; /****默认去掉了容易混淆的字符oOLl,9gq,Vv,Uu,I1****/
            var maxPos: number = $chars.length;
            var pwd: string = "";
            for (let i = 0; i < len; i++) {
                pwd += $chars.charAt(Math.floor(Math.random() * maxPos));
            }
            return pwd;
        }

        private Encrypt(word: string, randomKey: string): string {
            var srcs = CryptoJS.enc.Utf8.parse(word);
            var encrypted = CryptoJS.AES.encrypt(
                srcs,
                CryptoJS.enc.Utf8.parse(randomKey),
                {
                    mode: CryptoJS.mode.ECB,
                    padding: CryptoJS.pad.Pkcs7
                }
            );
            return encrypted.toString();
        }

        private Decrypt(word, randomKey): string {
            var decrypt = CryptoJS.AES.decrypt(word, CryptoJS.enc.Utf8.parse(randomKey), {
                mode: CryptoJS.mode.ECB,
                padding: CryptoJS.pad.Pkcs7
            });
            return CryptoJS.enc.Utf8.stringify(decrypt).toString();
        }

        //加密
        private encryption(params): Object {
            let randomStr, data, encrypt, encrypted;
            this.cryCommon();
            randomStr = this.randomString(16);
            // aes加密
            data = this.Encrypt(JSON.stringify(params), randomStr);
            // rsa加密
            encrypt = new JSEncrypt();
            encrypt.setPublicKey(
                "MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCJ9s1qlOyv9qpuaTqauW6fUftzE50rVk3yVPZwv1aO1Ch/XSEz76xCwkyvqpaqceRXrPpdBmO5+ruJ+I8osOHo7L5GWEOcMOO+8izp9hXKBBrmRMD4Egpn00k9DhVIEKp/vyddZPS/doxB8onhN6poTJDLdFLFVEicMf52caN9GQIDAQAB"
            );
            encrypted = encrypt.encrypt(randomStr);
            //创建json对象
            return {
                requestData: data,
                encrypted: encrypted
            };
        }
        //解密
        private DecryptData(res): any {
            this.cryCommon();
            let decrypt = new JSEncrypt();
            decrypt.setPrivateKey(
                ""
            );
            let aesKey = decrypt.decrypt(res.encrypted);
            // 用aes秘钥进行解密
            return JSON.parse(this.Decrypt(res.requestData, aesKey));
        }


        /**
         * 请求完成后执行
         * @param resp 
         */
        private onRequestComplete(data: any): void {
            if (this.decrypt) {
                data = this.DecryptData(data);
            };
            if (this.callback) {
                this.callback.apply(this.caller, [data]);
            } else {
                console.log("onRequestComplete", this.ajax.url, data);
            }
        }
        /**
         * 请求异常时执行
         * @param error 
         */
        private onRequestError(error: any): void {
            console.log("onRequestError", error, JSON.parse(this.ajax.http.response));
            if (this.callback) {
                this.callback.apply(this.caller, [JSON.parse(this.ajax.http.response)]);
            }
        }
    }
}